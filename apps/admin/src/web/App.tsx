...
export class App extends React.Component<Record<string, never>, State> {
  ...
  render() {
    ...
    return <div className="app-shell"><aside className="sidebar"><a className="brand" href="#dashboard"><span className="brand-mark"><Icon name="link" size={24}/></span><span>Linro<small>LINK MANAGEMENT</small></span></a><div className="nav-label">{ui.common.workspace}</div><nav>{nav.filter(item => !item.scope || this.can(item.scope)).map((item, index) => <React.Fragment key={item.page}>{index === 4 && <div className="nav-label management">{ui.common.management}</div>}<button className={'nav-item' + (page === item.page ? ' active' : '')} onClick={() => void this.navigate(item.page)}><Icon name={item.icon}/>{this.pageLabel(item.page)}{page === item.page && <span className="nav-active-dot"/>}</button></React.Fragment>)}</nav></aside>
      <div className="main-shell"><header className="topbar"><div className="breadcrumb">{ui.common.workspace} <span>/</span> <strong>{session.site_name}</strong> <span>/</span> <strong>{this.pageLabel(page)}</strong></div><div className="topbar-actions">{this.renderLanguageSwitch()}<div className="identity"><span className="identity-role">{String(session.user.role).toUpperCase()}</span><span className="avatar">{String(session.user.email).slice(0, 1).toUpperCase()}</span><span className="identity-email">{session.user.email}</span><button className="text-button" onClick={() => { sessionStorage.removeItem('cf-links-dev-token'); location.assign(session.auth_kind === 'local' ? '/' : '/cdn-cgi/access/logout'); }}>{ui.common.signOut}</button></div></div></header>
      <main><div className="page-heading"><div><div className="eyebrow">{page === 'dashboard' ? ui.eyebrow.dashboard : `${ui.eyebrow.other} / ${this.pageLabel(page).toUpperCase()}`}</div><h1>{this.pageLabel(page)}</h1><p>{this.pageDescription(page)}</p></div><div className="heading-actions"><button className="icon-button" title={ui.common.refresh} aria-label={ui.common.refresh} disabled={busy} onClick={() => void this.loadPage()}><Icon name="refresh"/></button>{(page === 'links' || page === 'dashboard') && this.can('links:write') && <button className="primary" disabled={!this.state.domains.length || busy} onClick={() => this.openModal('link')}><Icon name="plus"/>{ui.topbar.createLink}</button>}{page === 'domains' && this.can('domains:write') && <button className="primary" onClick={() => this.openModal('domain')}><Icon name="plus"/>{ui.topbar.addDomain}</button>}{page === 'users' && <button className="primary" onClick={() => this.openModal('user')}><Icon name="plus"/>{ui.topbar.addUser}</button>}{page === 'tokens' && <button className="primary" onClick={() => this.openModal('token')}><Icon name="plus"/>{ui.topbar.createToken}</button>}</div></div>
  ...
  renderPage() {
    ...
    if (s.page === 'dashboard') return <>
      <section className="metric-grid">{ui.dashboard.metrics.map(([label, hint, icon], index) => <div className="metric" key={label}><div className="metric-label">{label}<span><Icon name={icon} size={19}/></span></div><strong>{[s.summary.links, s.summary.active, s.summary.expired, s.summary.domains][index] === undefined ? ui.common.notAvailable : number([s.summary.links, s.summary.active, s.summary.expired, s.summary.domains][index], s.locale)}</strong><small>{hint}</small></div>)}</section>
      <section className="panel"><div className="panel-heading"><div><h2>{ui.dashboard.chartTitle}</h2><p>{ui.dashboard.chartSubtitle}</p></div><span className="quiet-pill">{s.analytics?.available ? ui.dashboard.chartReady(Number(s.analytics.clicks ?? 0)) : ui.dashboard.chartPending}</span></div>{s.analytics?.available ? <TrafficChart rows={s.analytics.timeline ?? []} locale={s.locale}/> : <Empty title={ui.dashboard.analyticsConnect} detail={s.analytics?.reason ?? ui.dashboard.analyticsReading} action={<button onClick={() => void this.navigate('analytics')}>{ui.dashboard.viewAnalytics} <Icon name="arrow" size={16}/></button>}/>}</section>
      <section className="panel"><div className="panel-heading"><div><h2>{ui.dashboard.recentTitle}</h2><p>{ui.dashboard.recentSubtitle}</p></div><button className="text-button" onClick={() => void this.navigate('links')}>{ui.common.viewAll} <Icon name="arrow" size={16}/></button></div>{this.renderLinkTable(true)}</section>
    </>;
